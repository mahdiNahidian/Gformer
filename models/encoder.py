...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, epoch=None):
    ...
    def forward(self, x, attn_mask=None, epoch=None):
        ...
        if self.conv_layers is not None:
            for attn_layer, conv_layer in zip(self.attn_layers, self.conv_layers):
                x, attn = attn_layer(x, attn_mask=attn_mask, epoch=epoch)
                x = conv_layer(x)
                attns.append(attn)

            x, attn = self.attn_layers[-1](x, attn_mask=attn_mask, epoch=epoch)
            attns.append(attn)
        else:
